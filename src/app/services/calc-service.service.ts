...
export class CalcService {
  ...
  public getAlreadyPaidThisDay(
    journee: Journee,
    onlyFactures: boolean = false,
  ) {
  ...
  public getMineThisDay(journee: Journee) {
    ...
    if (journee.planning && journee.planning.planningprestas) {
      ...
      total += parseFloat('' + tot);
      if (journee.devis && journee.devis.prestas) {
        journee.devis.prestas.forEach((presta) => {
          if (presta.kilorly && !presta.nom.includes('renfort'))
            total += this.prestaService.calc(presta);
        });
      }
  ...
  public getMonthFacturesMissing(
    journees: Journee[],
    month: number,
    year: number,
  ) {
    ...
    data.forEach((journee) => {
      if (journee.devis && journee.devis.prestas) {
        let reste =
          this.getMineThisDay(journee) -
          this.getAlreadyPaidThisDay(journee, true);
        if (reste > 0) {
          factures.push({
            creation: journee.date,
            nom: journee.nom,
            solde: reste,
            id: journee.id,
          });
  ...
  public getEstimate(journees: Journee[], year: number, month: number) {
    return (
      this.getAlreadyPaid(journees, year, month) +
      this.getNotPaid(journees, year, month, true)
    );
  }
  public getTotal(journees: Journee[], year: number, month: number) {
    return (
      this.getAlreadyPaid(journees, year, month) +
      this.getNotPaid(journees, year, month)
    );
