...
export class AppComponent implements OnInit {
  ...
  getData() {
    ...
    if (this.safedev && isDevMode()) {
      ...
      this.http
        .get<any>(this.baseapi + 'cloeplanning.php?artiste=' + this.artiste)
        .subscribe((data) => {
  ...
  getMonthFactures() {
    let factures = this.occupiedDates
      ...
      .filter((facture) => {
        ...
        if (this.month)
          return (
            factureYear == this.year && factureMonth == this.monthIndex + 1
          );
        else return factureYear == this.year;
      });
  ...
  getFactureText(facture: any) {
    let txt = '';
    let jour = this.occupiedDates.find((date: any) =>
      date.factures.find(
        (fac: any) =>
  ...
  notPaid2() {
    ...
    dates.forEach((date: any) => {
      date.devis.prestas
        .filter((presta: any) => !presta.nom.includes('renfort'))
        .forEach((presta: any) => (total += parseFloat(this.calc(presta))));
    });
    dates.forEach((date: any) => {
      date.factures.forEach((facture: any) => {
  ...
  totalThunes(i: any = -1) {
        ...
        if (date.prestataires) {
          ...
          if (tot == 0) {
            if (date.planning && date.planning.planningprestas) {
              date.planning.planningprestas.forEach((presta: any) => {
                if (presta.presta != 0)
                  tot = tot + parseFloat('' + presta.prix);
              });
            }
          }
          total -= tot;
    ...
    return (
      parseInt(total) + '€ (' + parseInt('' + (total - total * 0.24)) + '€ net)'
    );
  }
